"use strict";
function allFromDoc(e) {
  casheUserDoc(e);
  try {
    makeTasksFromDoc(e), setupFieldsFromDoc(e);
  } catch (e) {
    console.warn("could not setup app pages from doc", e);
  }
}
function casheUserDoc(e) {
  (localStorage["user-data"] = JSON.stringify(e.data())), (userDocCache = e.data());
}
function doClassSelect() {
  let e = getClassJSON(),
    a = $('[data-role="task-info-tag"]').html(
      "<option disabled selected hidden value=''>Choose a Class</option>"
    );
  delete e.other, (e.other = "Other");
  for (let t of Object.keys(e)) a.append(`<option value="${t}">${e[t]}</option>`);
}
function setupFieldsFromDoc(e) {
  if (e.exists) {
    $("[data-auth-role='name']").text(e.data().name.split(" ")[0]), (userDocCache = e.data());
    try {
      let a = 0,
        t = 0;
      for (let o of e.data().tasks ? e.data().tasks : [])
        ((o.date && !parseInt(o.date)) ||
          (o.date &&
            new Date(o.date).getTime() <=
              new Date(new Date().toLocaleDateString("en-ca")).getTime() + day_ms)) &&
          (a++, o.is_completed && t++);
      let o = parseInt((100 * t) / a);
      (o = isNaN(o) ? 0 : o),
        $('[data-role="progress-percentage"]').text(o),
        $('[data-role="progress-bar"]').css("width", `${o}%`),
        $('[data-role="progress-completed"]').text(t),
        $('[data-role="progress-total"]').text(a);
    } catch (e) {
      console.error(e);
    }
  } else new ErrorToast("Error", "Userdoc does not exist", 3e3);
  doClassSelect();
}
