...
function sortByDate(e, t) {
  let a = !e.date || e.is_pinned || "priority" == e.date || "top" == e.date,
    s = a || (!t.is_raised && new Date(e.date).getTime() < new Date(t.date).getTime());
  return s ? -1 : 1;
}
function pinTask(e) {
  try {
    (taskData = JSON.parse(e.attr("data-task-json-content"))),
      (task_is_pinned = e.hasClass("pinned"));
    let t = db.collection("users").doc(auth.currentUser.uid),
      a = db.batch();
    a.update(t, { tasks: firebase.firestore.FieldValue.arrayRemove(taskData) }),
      a.update(t, {
        tasks: firebase.firestore.FieldValue.arrayUnion({
          ...taskData,
          is_pinned: !task_is_pinned,
        }),
      }),
      a
        .commit()
    ...
    return new ErrorToast(
      `Could not ${e.hasClass("pinned") ? "un" : ""}pin task`,
      t.toString().includes("uid") ? "authentication has not been established yet" : cleanError(t),
      2e3
    );
  }
}
function makeTasksFromDoc(e) {
  if (e.exists) {
    let t,
      a = e.data().tasks,
      s = $('<div data-role="tasks-list"></div>'),
      i = $('<div id="todo-container" data-role="todo-container"></div>'),
      o = !1,
      n = 0;
    if (a) {
      a.forEach((e) => {
        e.title || (e.title = "Untitled Task"),
          (e.title_clean = e.title
            ...
            .replace(/"/g, "&quot;")
            .replace(/'/g, "&apos;")),
          (e.title_clean = e.title_clean.replace(
            /[^a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/? ]/g,
            ""
          ));
      }),
        ...
        a.forEach((e) => {
          if (
            (!do_old_complete || (do_old_complete && !e.is_completed)) &&
            (!e.is_completed ||
              (e.date && new Date(e.date).getTime() > new Date().getTime() - day_ms))
          ) {
            let a;
            e.iframe_url
              ? ((o = !0),
                (a = `\n            <div class="iframe-content task-card-loading task-card-content" style="background: ${
                  e.iframe_bg ? e.iframe_bg : "ffffff"
              ...
              : (a = `\n            <div class="task-card-content"><div class="task-card-widgets"><div class="task-card-time-widget"><object class="task-card-widget-icon" aria-label="Task Icon" data="../img/icon/tasks/clock-icon.png?was-svg" type="image/png"><img alt="icon" src="../img/icon/tasks/clock-icon.png" /></object><span class="task-card-time">${
                  ...
                  e.date && "priority" != e.date ? e.date : "No Goal Date"
                }</span></div></div><hr /><div class="task-card-info"><div class="task-card-title" title="${
                  e.title
                }">${e.title_clean}</div><div class="task-card-tag">${
                  Object.keys(getClassJSON()).includes(e.tag) ? getClassJSON()[e.tag] : e.tag
              ...
              (t = getDateText(e.date, e.is_pinned)),
              n != t &&
                ($("<div></div>", {
                  class: "task-section-header",
                  text: t,
                  "data-date": t,
                  timestamp: e.is_pinned ? "Pinned" : e.date ? e.date : "",
                }).appendTo(s),
                (n = t)),
              delete e.title_clean,
              $(
                `<div><div class='session-task-card-title'>${
                  e.title ? e.title : "No Title / iframe"
                }</div></div>`
              )
                .attr({ class: "session-task-card", "data-task-json-content": JSON.stringify(e) })
                .appendTo(i),
              $(
                `<div class="task-card${e.iframe_url ? " task-iframe-card" : ""}${
                  e.is_pinned ? " pinned" : ""
                }${
                ...
                .attr({
                  "data-task-json-content": JSON.stringify(e),
                  style: e.iframe_url ? "background: " + e.iframe_bg : "",
                })
                .appendTo(s);
          }
        }),
        s.children(".task-section-header").each(function () {
          let e = $(this);
          e.nextUntil(".task-section-header")
            .addBack()
            .wrapAll(
              `<div class='task-section' data-date='${e.text()}' data-timestamp='${e.attr(
                "timestamp"
              )}'></div>`
            );
        });
      let e = s.children(".task-section").first();
      (!e.attr("data-timestamp").includes("-") ||
        new Date(e.attr("data-timestamp")).getTime() > new Date().getTime() + day_ms) &&
        e.before(section_break),
        s.children(".task-section").each(function () {
          let e = $(this),
            t = e.next(".task-section");
          if (t.length) {
            let a = new Date(t.attr("data-timestamp")).getTime(),
              s = new Date(e.attr("data-timestamp")).getTime();
            (a - s > day_ms || !new Date(s)) && e.after(section_break);
          }
        });
    }
    if ($(s).text() != $("[data-role='tasks-list']").text()) {
      console.log("TASKSLIST: replacing"),
        $("[data-role='tasks-list']").replaceWith(s),
        $("[data-role='todo-container']").replaceWith(i),
        o ? $("[data-role='vite-add-card']").hide() : $("[data-role='vite-add-card']").show(),
      ...
      try {
        ...
        console.warn("could not setup swipe events", e);
      }
    } else console.log("TASKSLIST: matched");
  }
}
function getDateText(e, t = !1) {
  if (e && "priority" != e) {
    if ("top" == e || t) return "Pinned";
    if (new Date().toISOString().split("T")[0] == e) return "Today";
    {
      let t = new Date(e).toLocaleDateString("en-us", {
          weekday: "long",
          month: "long",
          day: "numeric",
          timeZone: "UTC",
      ...
      return t.split(" ").slice(0, 2).join(" ") + " " + a + i;
    }
  }
  return "Priority";
}
const section_break = '<div class="time-section-break">Nothing Planned</div>',
  day_ms = 864e5;
params.has("fullpage") &&
  "true" == params.get("fullpage") &&
  $(document.body).addClass("full-width-list"),
  $(".nav-item").click(function () {
    $(".nav-item").removeClass("active"),
      $(this).addClass("active"),
      $("#carousel").css({ left: 100 * -$(this).index() + "vw" }),
  ...
  $('[data-role="create-task"]').click(function () {
    ...
    if (Object.values(e).includes("")) new WarningToast("Please fill out all fields", 3e3);
    else {
      e.date = $('[data-role="task-info-date"]').val();
      try {
        e.date = e.date ? new Date(e.date).toISOString().split("T")[0] : "";
      } catch (t) {
        e.date = "";
      }
      db.collection("users")
        .doc(user.uid)
        .update({ tasks: firebase.firestore.FieldValue.arrayUnion(e) })
        .then(() => {
          $(
            '[data-role="task-info-title"], [data-role="task-info-tag"], [data-role="task-info-date"]'
          ).val(""),
            $("#time-30").prop("checked", !0),
            new Toast(
              "Task created!",
              "default",
              1e3,
        ...
        .catch((e) => {
          new ErrorToast("Could not save task to userdoc", cleanError(e), 2e3);
        });
    }
  }),
  $("#add .card *").on("change input click", function () {
    $('[data-role="task-info-title"]').val() &&
    ...
    $(this).closest(".task-card").addClass("swipe-out"),
      $(this)
        ...
        .animate({ left: "-100%" }, 500, function () {
          try {
            let e = JSON.parse($(this).attr("data-task-json-content"));
            $(this).remove(),
              db
                .collection("users")
                .doc(user.uid)
                .update({
                  tasks: firebase.firestore.FieldValue.arrayRemove(e),
                  archive: firebase.firestore.FieldValue.arrayUnion(e),
                })
                .then(() => {
                  new Toast(
                    "Task archived!",
            ...
            new ErrorToast("Could not archive task", cleanError(e), 2e3, ".");
          }
        });
  }),
  $(document.body).on("click", ".task-card-swipe-done", function () {
    $(this).closest(".task-card").addClass("completed").removeClass("editing");
    let e = do_old_complete ? { left: "-100%" } : { top: "0%" };
    $(this)
      .closest(".task-card")
      .animate(e, 500, function () {
        try {
          let e = JSON.parse($(this).attr("data-task-json-content")),
            t = JSON.parse($(this).attr("data-task-json-content"));
          if (((t.is_completed = !0), do_old_complete && $(this).remove(), e != t)) {
            let a = db.collection("users").doc(user.uid),
              s = db.batch();
            s.update(a, { tasks: firebase.firestore.FieldValue.arrayRemove(e) }),
              do_old_complete ||
                s.update(a, { tasks: firebase.firestore.FieldValue.arrayUnion(t) }),
              s
                .commit()
                .then(() => {
                  new Toast(
          ...
          } else console.log("Task already marked as done");
        } catch (e) {
          new ErrorToast("Could not mark task as done", cleanError(e), 2e3, ".");
        }
      });
  }),
  ...
  $(document.body).on(
    ...
    function () {
      let e = $(this).attr("data-save-archive") || $(this).hasClass("DATA-save-archive"),
        t =
          $(this).attr("DATA-clear-completed-old-tasks") ||
          $(this).hasClass("DATA-save-old-archive");
      db.collection("users")
        .doc(user.uid)
        .get()
        .then((a) => {
          ...
          if (
            (t &&
              (s = $.grep(s, function (e) {
                return new Date(e.date).getTime() + day_ms < new Date().getTime();
              })),
            s.length > 0)
          ) {
            let t = $.grep(s, function (e) {
                return e.is_completed;
              }),
              a = { tasks: firebase.firestore.FieldValue.arrayRemove(...t) };
            e && (a.archive = firebase.firestore.FieldValue.arrayUnion(...t)),
              db
                .collection("users")
                .doc(user.uid)
                .update(a)
                ...
                .catch((e) => {
                  new ErrorToast("Could not clear completed tasks", cleanError(e), 2e3, ".");
                });
          }
        });
    }
  ...
  $('[data-role="add-vite-card"]').click(function () {
    db.collection("users")
      ...
      .catch((e) => {
        new ErrorToast("Could not save VITE! card data userdoc", cleanError(e), 2e3);
      });
  }),
  $('[data-role="task-info-date"]').attr({
    min: new Date().toISOString().split("T")[0],
    max: new Date(new Date().getTime() + 31536e6).toISOString().split("T")[0],
  });
try {
